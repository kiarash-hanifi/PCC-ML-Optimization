...
#This just shortens the path you need to call for Streams and Blocks
    ...
    @property
    def CALC(self):
        """Property: Defines Path to the Calculator node in Aspen File system. 
            
        Aspendocument is defined in the Class Simulation initialization
        """
        return self.AspenSimulation.Tree.Elements("Data").Elements("Flowsheeting Options").Elements("Calculator")
...
#Related to Placing Blocks, connecting, removing and such things:
    ...
    def BlockPlace(self, Blockname:str, EquipmentType: Literal["RCSTR", "RPlug", "DSTWU", "Flash2", "Mixer", "Makeup", "Cooler" , "Heater", "Radfrac", "Splitter", "RYield"])-> None:
        """Adds only a BLOCK with given Name on the Aspen Simulation Sheet.
            ...
            Args:
                ...
                EquipmentType: Name of Equipment in Aspen. Can be: "RCSTR", "RPlug", "DSTWU", "Flash2", "Mixer", "Makeup", "Heater", "Radfrac", "Splitter", "RYield", 
        """
...
##MIXER:
    ...
    def BLK_MAKEUP_GET_ME_ALL_INPUTS_BACK(self, Blockname:str) -> Dict[str, Union[str,float,int]]:
        """Retrieves all the Inputs and returns Dictionary with Values 
        ...
        MassFlow = self.BLK.Elements(Blockname).Elements("Input").Elements("OUTMAFL").Value

        InumberList = [] # Children of MASSFRAC node (#0 in Aspen PLus)
        InumberNode = self.BLK.Elements(Blockname).Elements("Input").Elements("MASSFRAC").Elements
        for Inumber in InumberNode:
            InumberList.append(Inumber)
            MassFrac = self.BLK.Elements(Blockname).Elements("Input").Elements("MASSFRAC").Elements(Inumber).Value
            MassFracList.append(MassFrac)

        
        Dictionary = {
        "MassFracList": MassFrac    ,
        "MassFlow" :  MassFlow  ,
        "InumberList" : InumberList ,
        }
        return Dictionary
        
    def BLK_MAKEUP_SET_ALL_INPUTS(self, Blockname:str, Dictionary: Dict[str, Union[str,float,int]]) -> None:
        """Takes Dictionary with Values set the ones which are given in Aspen. 
        ...
        self.BLK.Elements(Blockname).Elements("Input").Elements("OUTMAFL").Value = Dictionary.get("MassFlow") 

        InumberList = Dictionary.get("InumberList")
        MassFracList = Dictionary.get("MassFracList")
        i = 0
        for Inumber in InumberList:
            self.BLK.Elements(Blockname).Elements("Input").Elements("MASSFRAC").Elements(Inumber).Value = MassFracList[i]
            i = i + 1
        i = 0 

##MAKEUP:
    def BLK_MAKEUP_Set_MassFrac(self, Blockname:str, MassFrac:float, Inumber):
        self.BLK.Elements(Blockname).Elements("Input").Elements("MASSFRAC").Elements(Inumber).Value = MassFrac
        # self.BLK.Elements(Blockname).Elements("Input").Elements("MASS_TOTAL").Value = MassFrac
    def BLK_MAKEUP_Set_MassFlow(self, Blockname:str, MassFlow:float):
        self.BLK.Elements(Blockname).Elements("Input").Elements("OUTMAFL").Value = MassFlow
    ...
    def BLK_COOLER_TP_GET_ME_ALL_INPUTS_BACK(self, Blockname:str) -> Dict[str, Union[str,float,int]]: # Only for the "TP" calculation type
        """Retrieves all the Inputs and returns Dictionary with Values 
    ...
    def BLK_COOLER_TP_SET_ALL_INPUTS(self, Blockname:str, Dictionary: Dict[str, Union[str,float,int]]) -> None:
        """Takes Dictionary with Values set the ones which are given in Aspen. 
...
##COOLER:
    def BLK_COOLER_TP_Set_Temperature(self, Blockname:str, Temperature:float):
        self.BLK.Elements(Blockname).Elements("Input").Elements("TEMP").Value = Temperature
        # self.BLK.Elements(Blockname).Elements("Input").Elements("MASS_TOTAL").Value = MassFrac
    def BLK_COOLER_TP_Set_Pressure(self, Blockname:str, Pressure:float):
        self.BLK.Elements(Blockname).Elements("Input").Elements("PRES").Value = Pressure
...
#Page 1         Flash specification
    ...
    def BLK_HEATER_Set_Temperature(self, Blockname:str, Temperature:float):
        self.BLK.Elements(Blockname).Elements("Input").Elements("TEMP").Value = Temperature
...
# Output for the Calculator 
    # This block should be customized based on the structure of the defined Calculator in the Simulation
    
    def CALC_Get_OUTPUTS(self, Blockname:str) -> Dict[str, float]:
        """Retrieves all Output variables for given Block and returns Dictionary of Values
            
            Args:
                Blockname: String which gives the name of Calculator Block.         
        """
        OutputVal = self.CALC.Elements(Blockname).Elements("Output").Elements("WRITE_VAL").Elements("1").Value 
        # The "1" in the above line, should be customized based on the order of variable defining (or it can be introduced as an input of the function)
...
#MIXER
#  PAGE 1 Summary
    def CALC_Get_OutputVal(self, Blockname):
        return self.CALC.Elements(Blockname).Elements("Output").Elements("WRITE_VAL").Elements("1").Value
...
############################################################################################################################################
    ...
    # My Function for Run
    # def RunWithTimeout(self, timeout) -> bool:
    #     """Runs simulation, if it takes more than the input timeout the simulation run will be stopped and reinitialized"""
    #     converged = 1 # if the value is empty for converged this will make it still unconverged

    #     self.AspenSimulation.Engine.Run2(1)  # run asynchronously
    #     start = time.time()
        
    #     while self.AspenSimulation.Engine.IsRunning:
    #         elapsed = time.time() - start
    #         print(f"Runtime so far = {elapsed:.1f} s", end="\r")
            
    #         if elapsed > timeout:
    #             print(f"\n⏱ Timeout > {timeout}s, stopping run...")
    #             self.AspenSimulation.Engine.Stop() # Stop the simulation
    #             converged = 0
    #             # break
    #         time.sleep(10)
    ...
    #     # conv_stat = self.AspenSimulation.Tree.Elements("Data").Elements("Results Summary").Elements(
    #                     # "Run-Status").Elements("Output").Elements("PER_ERROR").Value # reading convergence status from the simulation
    ...
    #     except Exception:
    #         self.AspenSimulation.Reinit()
    #         return False # If the node doesn't exist due to stop in the simulation

    #     if converged == 1 and conv_stat == 1:
    #         converged = True
    #     else:
    #         converged = False
    #         self.AspenSimulation.Reinit()      # Reinitialization
    ...
    def RunWithTimeout(self, timeout) -> bool:
        """Runs simulation, if there is a problem it will rerun twice, returns boolian about successful convergence"""
        # tries = 0
        converged = 0 # if the value is empty for converged this will make it still unconverged
        # timeout = 100
        #iterations = 10
        #self.BLK.Elements("B1").Elements("Input").Elements("MAXOL").Value = iterations
        ...
        # self.AspenSimulation.Run2()
        self.AspenSimulation.Engine.Run2(1)  # this 1 makes the run asynchronously
        start = time.time()
